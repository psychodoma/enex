...
if (preg_match('/admin\/login\/login\.php$/',$_ref['path']) || $_POST['adm_login']) {
	...
	if ($alStat == 'failure') {
		...
		msg('관리자 휴대폰 보안 인증을 먼저 하세요.', $certUrl, $returnTarket);
...
if( date("Y-m-d H:i:s", strtotime('-15 minutes')) > $result_db_login['account_lock_time'] ){
	...
	if($result_db_login['account_lock_yn'] == 'Y' && !$result_db_login['account_lock_time']){
		msg('접속이 제한 되어있습니다. 관리자에게 문의하세요.', $returnBack);
	}

}else{

	if($result_db_login['account_lock_yn'] == 'Y'){
		msg('로그인을 실패하여 10분 동안 접속이 제한됩니다.', $returnBack);
	}
}
...
if ($_POST['mode']=="guest"){ // 비회원 주문목록보기
	...
	if(count($chk_result)) {
		msg("주문자명과 주문번호가 일치하는 주문이 존재하지 않습니다", $returnBack);
	}
	...
	if($result!==true) {
		if (class_exists('loginFailCheck') === true && ($msg=loginFailCheck::msgSetting($result)) != '') {
			if ($result == 'LFC_adminLimitMsg') { // 주 관리자 > 5회 실패 메시지 (관리모드)
			...
			msg('아이디 또는 비밀번호 오류입니다', $returnBack);
		}
		elseif($result==='NOT_ACCESS') {
			msg('고객님은 본 사이트에서 승인되지 않아 로그인이 제한됩니다.', $returnBack);
		}
		else if($result==='NOT_VALID') {
			msg('아이디 또는 비밀번호 입력 형식 오류입니다', $returnBack);
		}
		exit;
	}
...
var _ag   = '' ;         // 로그인사용자 나이
var _id   = {_sess.m_id};    		// 로그인사용자 아이디
var _mr   = '';        	// 로그인사용자 결혼여부 ('single' , 'married' )
var _gd   = {_sess.sex};         // 로그인사용자 성별 ('man' , 'woman')
