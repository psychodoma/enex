...
<table cellpadding="0" cellspacing="0">
	...
	<? while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) { ?>
		<tr>
			...
			<td>
				<?php if($row['prd_etc']){?>
					<a href="/global<?get_prd_table_id($row['prd_kitchen_table'],$row['prd_kitchen_idx']) ?>" onClick="window.open(this.href, '', 'width=1000, height=550'); return false;"><?=$row['prd_etc']?></a>
				<?}?>
			</td>
		</tr>
	<? } ?>
